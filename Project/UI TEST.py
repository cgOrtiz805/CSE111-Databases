...
def Insert_Mon_But():
    ...
    def namesearch():
        ...
        egg_name = create.fetchall()[0]
        print(str(egg_name[0]))
        try:  # INSERTS VALUES IN Breeding TABLE
            create.execute("INSERT INTO Breeding VALUES(" + "'" + str(pokemonKey) + "'" + ", '" + str(egg_name[0])+ "','" + str(male) + "','" + str(female) + "','" + str(cycle) + "'" + ");")
            db_connection.commit()
        except sqlite3.Error as e:
            print(e)
    ...
    Label(root,text = "Height(ft)").grid(row=1)
    e1 = Entry(root)
    e1.grid(row=1, column =1)

    Label(root, text="Weight(lbs)").grid(row=2)
    e2 = Entry(root)
...
#Returns Entire Dex
def view_pokemon_but():
    root = Tk()
    root.title("Viewing All Pokemon")
    ...
    db_connection = sqlite3.connect(db_name)
    create = db_connection.cursor()
    try: #Returns all data for all pokemon
        create.execute("SELECT * FROM (SELECT p_PokeID,p_name, height, weight,h_name,"
                       " pt_type, st_type, s_name,a_name, eg_name,HP,Attack, Defense, SP_Atk, Speed, Male, Female, Egg_cycle "
        ...
        for row in mons:
            print(row)
        for i in mons:#Returns the data in a list box for the scroll bar
            listbox.insert(END, str(i))
        listbox.pack(side=LEFT, fill=BOTH, expand = 1000)
...
def name_search_but():
    ...
    def namesearch():
        ...
        try:
            ...
            for i in pokemon:
                str(i)
            mon_label = Label(root, text = i) #Need to add Labels
            mon_label.grid(row=4, columnspan =2)
        except sqlite3.Error as e:
            print(e)
    Label(root, text = "Pokemon Name").grid(row=0)
    e1 = Entry(root)
    ...
    submit = Button(root, text = "Submit", command = namesearch)
    submit.grid(row=2, columnspan=2, pady=10, padx=10, ipadx=100)
    root.mainloop()
...
def menu(db_connection):
    ...
    insert=Button(root, text = "Add a Pokemon",command = Insert_Mon_But)    #Calls The insert Button
    insert.grid(row = 0, columnspan = 2, pady = 10, padx=10, ipadx = 100)
    view_dex = Button(root, text="View all Pokemon", command=view_pokemon_but) #Calls view Pokemon Button
    view_dex.grid(row=1, columnspan=2, pady=10, padx=10, ipadx=100)
    search_name=Button(root, text = "Search by Name",command = name_search_but) # Calls Search by name Button
    search_name.grid(row = 2, columnspan = 2, pady = 10, padx=10, ipadx = 100)
